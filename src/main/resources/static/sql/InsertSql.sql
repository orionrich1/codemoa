...
('admin', 'https://www.programmers.co.kr/pages/프로그래머스_코드챌린지', 'TEAM_RECRUIT', '2025-01-20 ~ 2025-02-05', '2025-02-10 ~ 2025-05-01', 4, 2, '온라인', 'kakao:mia13', '#C++ #Go', 'RECRUITING', 'GitHub 링크 제출', '코드챌린지 풀스택 팀원 모집', '프론트 및 백엔드 개발 가능자 모집합니다.', NULL, 16, '2025-01-21 10:00:00', '2025-01-22 10:30:00'),
...
('user01', '신입 백엔드 면접에서 나온 질문 정리', '트랜잭션 격리 수준, 인덱스 설계, REST vs RPC 차이를 물어보셨어요. 혹시 최근 보신 분들은 어떤 질문 받으셨나요?', '자유', 'NORMAL', 12, NOW() - INTERVAL 1 DAY),
('user02', '코드리뷰에서 지적받을 때 마음 다잡는 법', '스타일이나 네이밍으로 코멘트 달릴 때 방어적으로 답장하지 않으려면 어떻게 하시나요? 선배 개발자분들 팁 부탁드립니다.', '자유', 'NORMAL', 8, NOW() - INTERVAL 2 DAY),
('user03', '레거시 터치하기 전 체크리스트 공유', '로그/모니터링 유무, 테스트 범위, 롤백 계획부터 정리하는 편입니다. 빠뜨리기 쉬운 항목 있으면 알려주세요.', '자유', 'NORMAL', 15, NOW() - INTERVAL 3 DAY),
('user04', 'VS Code vs IntelliJ 메인으로 뭐 쓰세요?', '풀스택이면 둘 다 쓰는 분들 많던데, JVM 위주면 IDEA가 편한 것 같아요. 의견 궁금합니다.', '자유', 'NORMAL', 21, NOW() - INTERVAL 4 DAY),
('user05', '원격근무 때 집중 유지 루틴', '뽀모도로 25분 + 스탠딩 책상 1시간에 한 번 일어나기 조합 씁니다. 재택 오래 하신 분들 루틴 있으면 공유 부탁해요.', '자유', 'NORMAL', 6, NOW() - INTERVAL 5 DAY),
('user06', '사이드 프로젝트 도메인 이름 고르는 기준', '너무 길면 기억 안 나고, 짧으면 .com 없을 때가 많아서 고민이에요. 후기나 도구 추천 있을까요?', '자유', 'NORMAL', 9, NOW() - INTERVAL 6 DAY),
('user07', '주니어가 첫 PR 올릴 때 본문에 꼭 넣는 것', '변경 요약, 스크린샷, 리스크/롤백 방법 정도 적는데 과한가요? 팀마다 문화가 다른 것 같아서요.', '자유', 'NORMAL', 11, NOW() - INTERVAL 7 DAY),
('user08', '기술 블로그 주 1회 쓰기 도전 중', '주제는 짧은 트러블슈팅 위주로 잡았어요. 같이 하실 분 있으면 서로 링크 공유해요.', '자유', 'NORMAL', 4, NOW() - INTERVAL 8 DAY),
('user09', '회사 맥북 개인 계정 Apple ID 어떻게 쓰시나요?', '업무용과 분리하려면 조직 계정이 있어야 하는지, 실무 경험 듣고 싶습니다.', '자유', 'NORMAL', 18, NOW() - INTERVAL 9 DAY),
('user10', '올해 읽은 개발 서적 베스트 1권', '저는 시스템 설계 인터뷰 준비용으로 본 책이 가장 도움 됐어요. 각자 올해 pick 공유해 주세요.', '자유', 'NORMAL', 7, NOW() - INTERVAL 10 DAY),
('admin', '서버 점검 예정 안내', '월요일 02:00~04:00 점검 예정입니다. 해당 시간대 API·웹 접속이 잠시 불안정할 수 있습니다.', '자유', 'NORMAL', 3, NOW() - INTERVAL 11 DAY),
('user01', 'Git 브랜치 전략 trunk-based vs GitFlow', '스타트업은 trunk 쪽이 많다고 들었는데 실제로 어떤 기준으로 고르시나요?', '자유', 'NORMAL', 14, NOW() - INTERVAL 12 DAY),
('user02', '면접에서 "가장 어려웠던 버그" 답변 예시', '재현 어려운 레이스 컨디션 이야기로 갔는데 너무 길어졌어요. 적당한 길이 감이 안 잡힙니다.', '자유', 'NORMAL', 10, NOW() - INTERVAL 13 DAY),
('user03', '오픈소스 첫 기여 후기', '문서 오타 수정부터 시작했는데 리뷰어가 친절해서 동기부여 됐어요. 망설이시는 분들께 추천합니다.', '자유', 'NORMAL', 16, NOW() - INTERVAL 14 DAY),
('user04', 'Docker Compose 로컬 DB 버전 팀 맞추기', '.env.example에 이미지 태그 고정해 두고 README에 적어 두었습니다. 더 나은 방법 있으면 알려주세요.', '자유', 'NORMAL', 5, NOW() - INTERVAL 15 DAY),
('user05', '주니어 때 포트폴리오에 넣은 프로젝트 규모', 'CRUD 한 개 + 배포까지가 현실적인지, 너무 작아 보일까 걱정됩니다.', '자유', 'NORMAL', 22, NOW() - INTERVAL 16 DAY),
('user06', 'SQL 쿼리 슬로우 로그 보는 습관', 'EXPLAIN부터 찍고 인덱스 후보부터 적어두는 편인데, 운영 DB는 권한 때문에 막히는 경우도 있더라고요.', '자유', 'NORMAL', 6, NOW() - INTERVAL 17 DAY),
('user07', '스탠드업 미팅 15분 안에 끝내는 팁', '블로커만 먼저 말하고 나머지는 비동기로 넘기기로 팀 합의 봤습니다.', '자유', 'NORMAL', 4, NOW() - INTERVAL 18 DAY),
('user08', 'TypeScript strict 켜기 마이그레이션 순서', 'any부터 줄이고, noImplicitAny 다음에 strictNullChecks 순으로 갔어요. 경험담 공유 부탁드려요.', '자유', 'NORMAL', 13, NOW() - INTERVAL 19 DAY),
('user09', '해커톤 팀 빌딩 때 역할 나누기', '기획·디자인·백·프론 최소 인원 정해두고, API 스펙 먼저 합의하는 게 시간 절약이었어요.', '자유', 'NORMAL', 8, NOW() - INTERVAL 20 DAY),
('user10', '연봉 협상 때 레퍼런스 체크 질문', '이전 직장 동료 연락처를 달라고 하시던데, 보통 어떻게 대응하시나요?', '자유', 'NORMAL', 19, NOW() - INTERVAL 21 DAY),
('admin', '커뮤니티 가이드라인 리마인드', '비방·차별·저작권 침해 글은 삭제될 수 있습니다. 기술 토론은 존중과 근거로 부탁드립니다.', '자유', 'NORMAL', 2, NOW() - INTERVAL 22 DAY),
('user01', '에러 메시지 한글화 vs 영문 유지', '로그는 영문, 사용자-facing만 한글로 통일했습니다. 다국어 나가면 전부 키 기반으로 빼야겠죠.', '자유', 'NORMAL', 7, NOW() - INTERVAL 23 DAY),
('user02', '온콜 로테이션 첫 주 후기', '알람 세팅과 런북 링크를 슬랙 핀 걸어두니 마음이 좀 편했어요.', '자유', 'NORMAL', 5, NOW() - INTERVAL 24 DAY),
('user03', '코딩 테스트 Python vs Java 선택', '회사 스택이 JVM인데 코테만 파이썬 써도 무방한지 면접관마다 다른 것 같아요.', '자유', 'NORMAL', 17, NOW() - INTERVAL 25 DAY),
('user04', '사내 기술 공유 세션 발표 주제 추천', '최근 팀에서 도입한 도구 소개 vs 개인적으로 파본 기술 중 뭐가 반응이 좋을까요?', '자유', 'NORMAL', 6, NOW() - INTERVAL 26 DAY),
('user05', 'DB 마이그레이션 Flyway vs Liquibase', '신규 서비스 시작하는데 둘 중 팀 합의가 안 나서요. 실사용 장단점 듣고 싶습니다.', '자유', 'NORMAL', 11, NOW() - INTERVAL 27 DAY),
('user06', '리모트 온보딩 2주차 체감', '문서화 잘 된 팀은 환경 세팅 하루 만에 끝났어요. 체크리스트 템플릿 있으면 공유 부탁드려요.', '자유', 'NORMAL', 9, NOW() - INTERVAL 28 DAY),
('user07', '스프링부트 3.x 넘어가면서 겪은 이슈', 'javax → jakarta 패키지 일괄 치환 외에 숨은 호환성 이슈 있었는지 경험담 부탁드립니다.', '자유', 'NORMAL', 12, NOW() - INTERVAL 29 DAY),
('user08', 'API 버저닝 URL vs Header', '우리는 /v1 prefix로 갔는데 모바일 앱이랑 맞추려면 헤더 방식이 나을 수도 있을까요?', '자유', 'NORMAL', 8, NOW() - INTERVAL 30 DAY),
('user09', '개발자 컨퍼런스 발표 지원 고민', '주제는 있는데 슬라이드 퀄리티가 걱정입니다. 처음 발표하신 분들 팁 있을까요?', '자유', 'NORMAL', 5, NOW() - INTERVAL 31 DAY),
('user10', '레디스 캐시 TTL 정하는 기준', '도메인마다 다르겠지만 보통 stale 허용 범위부터 역산하시나요?', '자유', 'NORMAL', 10, NOW() - INTERVAL 32 DAY),
('admin', 'CodeMoa 베타 피드백 채널', '버그·개선 제안은 Q&A 또는 이 게시판 태그 [피드백]으로 남겨 주세요.', '자유', 'NORMAL', 4, NOW() - INTERVAL 33 DAY),
('user01', '클라우드 비용 첫 달 모니터링', '알람 임계값을 너무 낮게 잡아서 스팸 됐던 적 있어요. 권장 설정 있으면 공유해 주세요.', '자유', 'NORMAL', 7, NOW() - INTERVAL 34 DAY),
('user02', '페어 프로그래밍 도입 설득하기', '속도 느려진다는 우려가 있었는데 버그 감소로 역설득했습니다. 사례 더 있으면 좋겠어요.', '자유', 'NORMAL', 14, NOW() - INTERVAL 35 DAY),
('user03', '문서화 툴 Notion vs Confluence', '스타트업 10명 내외 기준으로 뭐가 덜 피곤한지 경험담 듣고 싶어요.', '자유', 'NORMAL', 15, NOW() - INTERVAL 36 DAY),
('user04', 'HTTPS 인증서 만료 알람 설정', 'Let''s Encrypt 자동 갱신 실패했을 때 메일 오게 해두었는데 다른 채널도 쓰시나요?', '자유', 'NORMAL', 6, NOW() - INTERVAL 37 DAY),
('user05', '주간 회고 템플릿 추천', 'Keep / Problem / Try 형식 쓰는데 Try가 매주 비슷해지는 느낌입니다.', '자유', 'NORMAL', 5, NOW() - INTERVAL 38 DAY),
('user06', '신규 입사자 첫 커밋 날짜', '온보딩 일정으로 3일째에 첫 PR 머지 목표로 잡았어요. 현실적인지요.', '자유', 'NORMAL', 9, NOW() - INTERVAL 39 DAY),
('user07', '장애 대응 후 포스트모템 문화', '비난 없이 타임라인·원인·재발방지만 적는 템플릿 쓰는데 잘 안착할 때 팁 있을까요?', '자유', 'NORMAL', 11, NOW() - INTERVAL 40 DAY);
...
('user01', '포트폴리오 랜딩 SaaS', '개발자용 링크인바이오 + 프로젝트 쇼케이스. Next.js 15, Stripe 결제 연동 목표.', '진행중'),
('user02', '실시간 협업 화이트보드', 'Canvas API와 WebSocket으로 원격 스터디용 보드. 팀 과제 MVP.', '진행중'),
('user03', '배달앱 클론 풀스택', '도메인 주도 설계 연습. 장바구니·결제 플로우까지.', '계획'),
('user04', '개인 재무 대시보드', '공공·샌드박스 API와 차트로 월별 지출 시각화.', '진행중'),
('user05', '사내 Wiki 검색 PoC', 'Elasticsearch 도입 검토. 제목·본문 동의어 사전 구성.', '종료'),
('user06', 'KMP 투두 앱', 'Kotlin Multiplatform으로 Android·iOS 동시 배포 실험.', '진행중'),
('admin', '중간 프로젝트', '반려동물 올인원 종합 케어 시스템 사이트 개발', '종료'),
('admin', '파이널 프로젝트', 'AI를 활용한 개발자 커뮤니티 사이트 개발', '진행중');

INSERT INTO project_checklist (project_id, content, is_done, priority) VALUES
(1, 'Vercel 배포 파이프라인 구축', TRUE, 1),
(1, 'OG 이미지 동적 생성', FALSE, 2),
(1, '방문자 분석 이벤트 설계', FALSE, 3),
(8, '요구사항 수집', TRUE, 2),
(8, '데이터베이스 설계', TRUE, 1),
(8, '백엔드 기본 구조 구현', FALSE, 1),
(8, '프론트엔드 메인 페이지 디자인', FALSE, 2),
(8, 'OAuth 로그인 연동 (Google, GitHub)', FALSE, 1),
(8, 'AI 문제 풀이 기능 1차 버전 구현', FALSE, 1),
(8, '프로젝트 협업 기능 초안 개발', FALSE, 2),
(8, '테스트 서버 배포', FALSE, 3);

INSERT INTO project_diary (project_id, work_date, content) VALUES
(1, '2026-03-15', '프로젝트 카드 그리드 레이아웃을 반응형으로 맞췄고, 다크 모드 토큰을 site-ds 변수로 통일했습니다.'),
(1, '2026-03-22', 'Waitlist 폼에 Turnstile 봇 방지 붙이고, Resend로 환영 메일 발송 연동했습니다.'),
(4, '2026-02-10', '월별 집계 쿼리 최적화. 인덱스 후보 컬럼 정리하고 EXPLAIN으로 검증했습니다.'),
(8, '2025-07-20', '요구사항을 정리하여 문서화 작업을 완료했습니다. 주요 기능 목록을 확정.'),
(8, '2025-07-25', '데이터베이스 기본 설계를 완료하고 ERD를 작성했습니다. 일부 테이블은 팀원 의견 반영 후 수정 예정.'),
(8, '2025-07-30', 'Spring Boot 기반의 백엔드 초기 구조를 세팅하고 공통 설정 코드를 작성했습니다.'),
(8, '2025-08-01', '프론트엔드 메인 페이지 시안을 제작하고, 디자인 툴로 공유하여 피드백을 받았습니다.'),
(8, '2025-08-03', 'Google OAuth 로그인 연동 테스트를 진행했으며, GitHub 연동은 에러 수정 후 재시도 예정.'),
(8, '2025-08-06', 'AI 문제 풀이 기능 1차 버전의 API 연동을 완료하고 간단한 테스트를 수행했습니다.'),
(8, '2025-08-09', '협업 기능(작업 다이어리, 체크리스트) 초안 UI를 구현했습니다.'),
(8, '2025-08-11', '테스트 서버에 첫 배포를 진행하고 주요 기능 동작 여부를 점검했습니다.');
...
('일반', '이 사이트는 어떤 목적으로 만들어졌나요?', '개발자들이 코딩 테스트 문제와 노하우를 공유하고, 협업할 수 있도록 돕기 위해 만들어졌습니다.'),
('계정', '회원 가입은 어떻게 하나요?', '상단 메뉴에서 회원 가입 버튼을 클릭해 이메일과 비밀번호를 입력하면 가입할 수 있습니다.'),
('문제·코딩', 'AI 코딩 문제 풀이 지원 기능은 어떻게 사용하나요?', '문제 풀이 페이지에서 질문을 입력하면 AI가 풀이 예제와 설명을 제공합니다.'),
('일반', '내 작업물을 어떻게 저장하고 관리할 수 있나요?', '개인 다이어리 기능과 Git 연동을 통해 버전 관리하며 저장할 수 있습니다.'),
('일반', '코딩 작업용 음악 플레이리스트는 어떻게 활성화하나요?', '개인 도구 메뉴에서 플레이리스트 탭을 선택해 원하는 음악을 재생할 수 있습니다.'),
('문제·코딩', '문제 풀이 중 모르는 내용이 있으면 어디서 질문하나요?', '커뮤니티 게시판에 질문글을 작성하거나, 댓글로 다른 사용자와 소통할 수 있습니다.'),
('일반', '프로젝트 협업 기능은 무엇인가요?', '팀원들과 작업을 분담하고 진행 상황을 공유하는 협업 도구를 제공합니다.'),
('일반', '포인트는 어떻게 획득하나요?', '문제 풀이 완료, 게시글 작성, 다른 사용자 도움 등 다양한 활동으로 포인트를 받을 수 있습니다.'),
('일반', '포인트는 어디에 사용할 수 있나요?', '포인트는 사이트 내 특별 콘텐츠 접근, 아이템 구매 등 다양한 용도로 활용할 수 있습니다.'),
('계정', '비밀번호를 잊어버렸을 때 어떻게 하나요?', '로그인 페이지에서 ‘비밀번호 찾기’ 기능을 통해 이메일 인증 후 재설정할 수 있습니다.'),
('일반', '모바일에서도 사이트를 사용할 수 있나요?', '네, 모바일 최적화가 되어 있어 스마트폰과 태블릿에서도 편리하게 이용할 수 있습니다.'),
('계정', '내 프로필 정보는 어떻게 수정하나요?', '로그인 후 마이페이지에서 닉네임, 이메일 등 프로필 정보를 변경할 수 있습니다.'),
('일반', '사이트 이용 중 오류가 발생하면 어떻게 신고하나요?', '고객센터 또는 피드백 메뉴를 통해 상세 내용을 작성해 보내주시면 빠르게 처리하겠습니다.');
...
-- 실시간 채팅 (JPA ChatRoom / ChatMessage). sentAt은 스케줄러가 6시간 이전 메시지를 삭제하므로 최근 시각으로 둠.
INSERT INTO ChatRoom (roomId, name) VALUES
('c1111111-1111-4111-a111-111111111101', 'Spring Boot·JPA Q&A'),
('c2222222-2222-4222-a222-222222222202', '알고리즘·코딩테스트'),
('c3333333-3333-4333-a333-333333333303', '취업·이력서·포트폴리오');

INSERT INTO ChatMessage (roomId, sender, message, sentAt, type) VALUES
('c1111111-1111-4111-a111-111111111101', 'user02', 'WebMvcTest에서 Security 빈 끼워 넣을 때 @Import만으로 부족한 케이스 있나요?', DATE_SUB(NOW(6), INTERVAL 280 MINUTE), 'TALK'),
('c1111111-1111-4111-a111-111111111101', 'user05', '@MockBean UserDetailsService 쓰면 됩니다. 슬라이스는 컨텍스트 얇아서 그쪽이 흔해요.', DATE_SUB(NOW(6), INTERVAL 275 MINUTE), 'TALK'),
('c1111111-1111-4111-a111-111111111101', 'admin', '공식 문서도 MockMvc + Security 조합 예제 참고해 보세요.', DATE_SUB(NOW(6), INTERVAL 270 MINUTE), 'TALK'),
('c1111111-1111-4111-a111-111111111101', 'user03', 'JPA N+1 막으려면 fetch join이랑 @EntityGraph 중 뭐부터 쓰시나요?', DATE_SUB(NOW(6), INTERVAL 120 MINUTE), 'TALK'),
('c1111111-1111-4111-a111-111111111101', 'user07', '목록 조회는 보통 EntityGraph로 시작하고, 복잡하면 프로젝션 DTO 쿼리로 빼요.', DATE_SUB(NOW(6), INTERVAL 115 MINUTE), 'TALK'),
('c1111111-1111-4111-a111-111111111101', 'user01', '감사합니다. 페이징이랑 같이 쓸 때 주의점 있을까요?', DATE_SUB(NOW(6), INTERVAL 110 MINUTE), 'TALK'),
('c1111111-1111-4111-a111-111111111101', 'user07', 'OneToMany fetch join + 페이징은 카테시안 주의. batch size나 두 단계 조회도 같이 검토하세요.', DATE_SUB(NOW(6), INTERVAL 105 MINUTE), 'TALK'),
('c1111111-1111-4111-a111-111111111101', 'user08', '헬스 체크 엔드포인트만 permitAll 나머지는 JWT로 가려는데 예제 구조 추천 부탁드려요.', DATE_SUB(NOW(6), INTERVAL 45 MINUTE), 'TALK'),
('c1111111-1111-4111-a111-111111111101', 'user04', 'SecurityFilterChain 두 개 쪼개서 /actuator/** 랑 /api/** 분리하는 패턴 많이 씁니다.', DATE_SUB(NOW(6), INTERVAL 40 MINUTE), 'TALK'),

('c2222222-2222-4222-a222-222222222202', 'user06', '이분 탐색 left/right 경계 줄일 때 while(lo<=hi) vs while(lo<hi) 언제 쓰시나요?', DATE_SUB(NOW(6), INTERVAL 260 MINUTE), 'TALK'),
('c2222222-2222-4222-a222-222222222202', 'user09', '문제마다 다르지만 상한 미포함이면 hi=mid-1 패턴이 익숙해요. 템플릿 하나 정해두고 맞추는 게 좋아요.', DATE_SUB(NOW(6), INTERVAL 255 MINUTE), 'TALK'),
('c2222222-2222-4222-a222-222222222202', 'user10', '그래프 BFS에서 방문 체크를 큐 넣을 때 할지 뺄 때 할지 팀 컨벤션 있나요?', DATE_SUB(NOW(6), INTERVAL 200 MINUTE), 'TALK'),
('c2222222-2222-4222-a222-222222222202', 'user03', '넣을 때 하는 쪽이 중복 방지에 안전한 경우가 많습니다.', DATE_SUB(NOW(6), INTERVAL 195 MINUTE), 'TALK'),
('c2222222-2222-4222-a222-222222222202', 'user02', '오늘 프로그래머스 Lv3 하나 러닝타임 터졌는데 Java에서 ArrayDeque 쓰니 통과했어요.', DATE_SUB(NOW(6), INTERVAL 90 MINUTE), 'TALK'),
('c2222222-2222-4222-a222-222222222202', 'user05', 'LinkedList는 코테에서 거의 안 쓰는 게 맞습니다 ㅎㅎ', DATE_SUB(NOW(6), INTERVAL 85 MINUTE), 'TALK'),
('c2222222-2222-4222-a222-222222222202', 'user06', '누적합 + 해시로 O(n) 만드는 유형 추천 문제 있을까요?', DATE_SUB(NOW(6), INTERVAL 25 MINUTE), 'TALK'),
('c2222222-2222-4222-a222-222222222202', 'user09', 'LeetCode 560, 백준 2003 클래스 비슷한 느낌으로 몇 번 풀어보면 감 잡혀요.', DATE_SUB(NOW(6), INTERVAL 20 MINUTE), 'TALK'),

('c3333333-3333-4333-a333-333333333303', 'user01', '백엔드 주니어 포트폴리오에 REST API 몇 개·배포 스크린샷 정도면 턱없이 부족할까요?', DATE_SUB(NOW(6), INTERVAL 240 MINUTE), 'TALK'),
('c3333333-3333-4333-a333-333333333303', 'user04', '트레이드오프랑 장애 대응 경험 한 줄이라도 있으면 설득력이 달라요. 수치(응답 시간, 비용) 있으면 더 좋고요.', DATE_SUB(NOW(6), INTERVAL 235 MINUTE), 'TALK'),
('c3333333-3333-4333-a333-333333333303', 'user07', '이력서에 기술 스택 나열 대신 “무엇을 개선했는지” 한 문장씩만 바꿔도 체감됩니다.', DATE_SUB(NOW(6), INTERVAL 180 MINUTE), 'TALK'),
('c3333333-3333-4333-a333-333333333303', 'user08', 'CS 질문 대비로 운영체제 프로세스/스레드랑 DB 트랜잭션만 짚고 가도 면접 버티기 쉬워요.', DATE_SUB(NOW(6), INTERVAL 175 MINUTE), 'TALK'),
('c3333333-3333-4333-a333-333333333303', 'user10', '코테 점수는 서류 통과용이고, 과제 퀄이 최종에 더 영향 준다는 후기 많더라고요.', DATE_SUB(NOW(6), INTERVAL 60 MINUTE), 'TALK'),
('c3333333-3333-4333-a333-333333333303', 'admin', '사이트 내 채용·팀 모집 메뉴랑 Q&A도 같이 보면 정보 모으기 좋습니다.', DATE_SUB(NOW(6), INTERVAL 55 MINUTE), 'TALK'),
('c3333333-3333-4333-a333-333333333303', 'user02', '면접 복기 글 올릴 때 회사명은 가리는 게 좋겠죠?', DATE_SUB(NOW(6), INTERVAL 15 MINUTE), 'TALK'),
('c3333333-3333-4333-a333-333333333303', 'user05', '네, NDA·이미지 이슈 때문에 보통 익명 처리합니다.', DATE_SUB(NOW(6), INTERVAL 10 MINUTE), 'TALK');

-- 주간 랭킹·활동 지표 (PointLog JPA ↔ CreateSql 테이블명 pointlog, 컬럼 camelCase)
INSERT INTO pointlog (user_id, points, description, eventType, created_at) VALUES
('user10', 15, '커뮤니티 기술 글 작성', 'CREATE_POST', '2026-04-07 10:15:00'),
('user10', 5, '질문글 댓글', 'CREATE_COMMENT', '2026-04-07 10:22:00'),
('user09', 15, '게시글 작성', 'CREATE_POST', '2026-04-07 09:30:00'),
('user09', 100, '일일 로그인', 'DAILY_LOGIN', '2026-04-07 08:05:00'),
('user09', 5, '댓글 작성', 'CREATE_COMMENT', '2026-04-07 09:00:00'),
('user08', 15, '게시글 작성', 'CREATE_POST', '2026-04-06 14:10:00'),
('user08', 5, '댓글 작성', 'CREATE_COMMENT', '2026-04-06 15:20:00'),
('user08', 5, '댓글 작성', 'CREATE_COMMENT', '2026-04-06 16:00:00'),
('user08', 100, '일일 로그인', 'DAILY_LOGIN', '2026-04-07 07:40:00'),
('user07', 100, '일일 로그인', 'DAILY_LOGIN', '2026-04-07 07:30:00'),
('user07', 15, '게시글 작성', 'CREATE_POST', '2026-04-07 12:05:00'),
('user07', 5, '댓글 작성', 'CREATE_COMMENT', '2026-04-06 18:00:00'),
('user06', 120, 'AI 코딩 문제 제출 보상', 'PROBLEM_SUBMIT', '2026-04-07 13:20:00'),
('user06', 15, '게시글 작성', 'CREATE_POST', '2026-04-06 19:00:00'),
('user05', 15, '게시글 작성', 'CREATE_POST', '2026-04-06 11:00:00'),
('user05', 15, '게시글 작성', 'CREATE_POST', '2026-04-07 15:10:00'),
('user05', 100, '일일 로그인', 'DAILY_LOGIN', '2026-04-06 09:00:00'),
('user04', 5, '댓글 작성', 'CREATE_COMMENT', '2026-04-07 16:00:00'),
('user04', 5, '댓글 작성', 'CREATE_COMMENT', '2026-04-07 16:08:00'),
('user04', 15, '게시글 작성', 'CREATE_POST', '2026-04-06 10:30:00'),
('admin', 100, '일일 로그인', 'DAILY_LOGIN', '2026-04-07 06:00:00'),
('admin', 15, '공지 성격 게시글', 'CREATE_POST', '2026-04-06 08:00:00');
